...
public class BookingFormController implements Initializable {
    ...
    void payButtonOnAction(ActionEvent event) {
        ...
        try {
            if (validation==0) {
                isPlaced = bookingModel.paymentBooking(bookingId, date, toDate, trainId, stationName, seatsId, paymentId, price, passengerId, passengerName, contactNum, email, address, Nic);
            }
            if(isPlaced) {
                value =1;
                validation=0;
